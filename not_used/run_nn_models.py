...
from model_utils import load_data, folds_choose_subjects, subject_data_inds, roi_proj_rf, str2bool, \
    get_custom_motor_rois, proj_mats_good_rois
...
def cnn_model(X_train, Y_train, X_validate, Y_validate, X_test, Y_test, chckpt_path, modeltype,
    ...
    model = htnet(nb_classes, Chans=X_train.shape[1], Samples=X_train.shape[2],
                  ...
                  ROIs=nROIs, useHilbert=useHilbert, do_log=do_log,
...
def run_nn_models(sp, n_folds, combined_sbjs, lp, roi_proj_loadpath,
                  pats_ids_in=None,
                  n_evs_per_sbj=500, test_day=None, tlim=None,
    ...
    if tlim is None:
        raise ValueError("ValueError 'tlim' not specified")
    if pats_ids_in is None:
        ...
        for modeltype in models:
            ...
            for frodo in range(n_folds):
                ...
                if modeltype == 'rf':
                    ...
                    accs_lst, last_epoch_tmp = cnn_model(X_train, Y_train, X_validate, Y_validate,
                                                         X_test3, y_test2, chckpt_path, modeltype,
                                                         nb_classes=nb_classes, dropoutRate=dropoutRate,
                                                         kernLength=kernLength, F1=F1, D=D, F2=F2,
                                                         dropoutType=dropoutType, kernLength_sep=kernLength_sep,
                                                         loss=loss, optimizer=optimizer,
                                                         patience=patience, early_stop_monitor=early_stop_monitor,
                                                         do_log=do_log,
                                                         epochs=epochs, compute_val=compute_val,
            ...
            np.save(
                sp + 'acc_' + modeltype + '_' + pat_id_curr + '_testday_' + str(test_day) + save_suffix + '.npy',
                accs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-tdy', '--test_day', required=False, type=str, default=None,
                        help='Test day to use (None or ''last'')')
    ...
    parser.add_argument('-mods', '--models', required=False, nargs='+', type=str,
                        default=['eegnet_hilb', 'eegnet', 'rf'],
    ...
    run_nn_models(args.save_path, args.n_folds, args.combined_sbjs, args.load_path, args.patient_ids,
                  args.n_evs_per_sbj,
                  ...
                  args.loss, args.optimizer, args.patience, args.early_stop_monitor, args.do_log, args.n_test,
                  args.n_val,
